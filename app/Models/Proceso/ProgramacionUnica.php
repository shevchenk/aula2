...
class ProgramacionUnica extends Model
{
    ...
    public static function runLoad($r)
    {
        $result=DB::table('v_programaciones_unicas AS pu')
                ...
                'vc.curso','vc.imagen','pu.curso_id',
                ...
                ->join('v_cursos as vc','vc.id','=','pu.curso_id')
                ->join('v_personas as vp','vp.id','=','pu.persona_id')
                ->leftjoin('v_contenidos as vco', function($join)use($r){
                    $join->on('vco.programacion_unica_id','=','pu.id')
                         ->where('vco.estado','=',1);
                })
                ->where(
                    function($query) use ($r){
                       $query->where('pu.estado','=',1);
                ...
                ->groupBy('pu.id','pu.plantilla','pu.fecha_inicio',
