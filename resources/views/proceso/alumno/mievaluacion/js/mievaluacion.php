    ...
    $.each(result.data.data,function(index, r){
        ...
        html+='<div class="col-lg-4">'+
          ...
          if(r.evaluacion_resultado.split("|")[2] == 0 && r.evaluacion_resultado.split("|")[0]==''){
            ...
            html+='<ul><li class="list-group-item list-group-item-'+colorrst+'" style="font-weight: bold;"><span class="badge" style="font:18px;">'+r.evaluacion_resultado.split("|")[3]+'</span>'+textorst+'</li></ul>';
          }
        html+='</div>'+
            '</div>'+
          '</div>';
...
verSiguientePregunta=function(id){
  ...
  else
  {
    if(CantevalG == (ceval + 1)) // Última pregunta seleccionado
    {
      ...
      $('#body-preguntas').html('<p class="bg-info text-center" style="font-size: 20px;">Gracias por completar su evaluación. Para finalizar haga click en "Procesar Examen" <br/> '+
